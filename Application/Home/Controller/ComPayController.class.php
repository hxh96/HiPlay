...
{
//证书
    private $apiclient_cert = './ThinkPHP/Library/Vendor/WxPayPubHelper/cert/apiclient_cert.pem';
    private $apiclient_key = './ThinkPHP/Library/Vendor/WxPayPubHelper/cert/apiclient_key.pem';
    //pay的秘钥值
    private $apikey = "JkiyKL2rainZPPsL7CIfDYYVVE3eIcxj";
    //错误信息
    private $error = '';

    private $mchid = '1503382621';//商户号
    private $mchappid = 'wx7b0e83e677e7f200';//公众号
    private $openid = 'ofgtxxMVYt37CGUJ6QClyXPqr9Y0';//接收者openid
    private $amount = 100;//金额
    private $partnertradeno = '';//订单号
    private $spbillcreateip = '';//触发ip
    private $checkname = 'NO_CHECK';//校验要求

    private $sendname = 'Hi玩';
    private $wishing = '再接再历';
    private $actname = 'Hi玩';
    private $remark = '再接再历';
     ...
     *@return boolean
     */
    public function RedBag($openid,$total_amount){
        if(!$this->inited()){
            return show(400,$this->error);exit;
        };
        $obj = array();
...
//        $obj['total_amount'] = $this->amount;
        $obj['total_amount'] = (int)$total_amount;
        $obj['total_num'] = 1;
        ...
        $url = $this->api_single;
//        var_dump($obj['client_ip']);exit;
        $xml = $this->Pay($url,$obj);
        $data =  json_decode(json_encode(simplexml_load_string($xml, 'SimpleXMLElement', LIBXML_NOCDATA)), true);
        //判断发送红包结果
        if($data['return_code'] == "FAIL"){
            return show(400,'通信出错:'.$data['return_msg']);
        }elseif($data["result_code"] == "FAIL"){
            return show(400,'业务出错:'.$data['err_code_des']);
        }else{
            return show(200,'发送成功',$data);
        }
//        var_dump($data);exit;
//        return ;
    }
...
//            $this->apiclient_cert = dirname(__FILE__)."/cacert/apiclient_cert.pem";
            $this->apiclient_cert = './ThinkPHP/Library/Vendor/WxPayPubHelper/cert/apiclient_cert.pem';
        if(!$this->apiclient_key)
//            $this->apiclient_key = dirname(__FILE__)."/cacert/apiclient_key.pem";
            $this->apiclient_key = './ThinkPHP/Library/Vendor/WxPayPubHelper/cert/apiclient_key.pem';
    }
...
//        var_dump($this->apiclient_cert);exit;
        $ch = curl_init();
