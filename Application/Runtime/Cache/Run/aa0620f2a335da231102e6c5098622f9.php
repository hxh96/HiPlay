...
<div class="public-ifame mt20">
	...
	<div class="public-content">
		...
		<div class="public-content-cont">
			...
			<div class="form-group">
				<label for="">发布者</label>
				<select name="initiate_user_id" class="form-select">
					<?php if(is_array($official)): $i = 0; $__LIST__ = $official;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($v["id"]); ?>"><?php echo ($v["user_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
				</select>
			</div>
			<div class="form-group">
				<label for="">标题</label>
				<input class="form-input-txt" type="text" placeholder="请输入标题" name="title" value="" />
			</div>
			<!--<div class="form-group">-->
				<!--添加间隔时间(单位:秒)-->
				<!--<input class="form-input-txt" type="text" placeholder="请输入添加间隔时间" id="sleepTime" name="sleepTime" value="" />-->
			<!--</div>-->
			<div class="form-group">
				<label for="">费用类型</label>
				<select name="activity_cate_id" class="form-select">
					<?php if(is_array($activityCate)): $i = 0; $__LIST__ = $activityCate;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($v["id"]); ?>"><?php echo ($v["activity_cate_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
				</select>
			</div>
			...
			<div class="form-group">
				...
				<select name="category_id" class="form-select">
					<?php if(is_array($category)): $i = 0; $__LIST__ = $category;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($v["id"]); ?>"><?php echo ($v["name"]); ?></option>
						<?php if($v[_child]): if(is_array($v["_child"])): $i = 0; $__LIST__ = $v["_child"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$z): $mod = ($i % 2 );++$i;?><option value="<?php echo ($z["id"]); ?>">---<?php echo ($z["name"]); ?></option>
								<?php if($z[_child]): if(is_array($z["_child"])): $i = 0; $__LIST__ = $z["_child"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$y): $mod = ($i % 2 );++$i;?><option value="<?php echo ($y["id"]); ?>">------<?php echo ($y["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
			...
			<div class="form-group">
				...
				<input class="form-input-txt" type="text" placeholder="请输入参与人数" name="join_number" value="" />
			</div>


			<div class="form-group">
				<label for="">活动说明</label>
				<textarea  name="content"  class="form-input-textara" style="width:465px;height:200px;"></textarea>
			</div>


			<div class="form-group">
				<label for="">开始时间</label>
				<input class="form-input-txt" name="start_time" placeholder="请选择" id="start_time" class="laydate-icon" onClick="laydate({elem: '#start_time',istime: true, format: 'YYYY-MM-DD hh:mm:ss'})">
			</div>

			<div class="form-group">
				<label for="">活动经度</label>
				<input class="form-input-txt" type="text" placeholder="请输入活动经度" name="activity_longitude" value="" />
			</div>
			<div class="form-group">
				<label for="">活动纬度</label>
				<input class="form-input-txt" type="text" placeholder="请输入活动纬度" name="activity_latitude" value="" />
			</div>
			...
			<div class="form-group" style="margin-left:150px;">
				...
				<button class="return-btn" ><a class="btn-a" href="<?php echo U('Activity/index');?>">返  回</a></button>
			</div>
		</div>
	</div>
	...
	<script>
		var SCOPE = {
			'redirect_url' : "<?php echo U('Activity/addData');?>",
