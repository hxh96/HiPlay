...
<div class="public-ifame mt20">
	...
	<div class="public-content">
		...
		<div class="public-content-cont">
			...
			<div class="form-group">
				...
				<input class="form-input-txt" type="text" placeholder="请输入分类名称" name="name" value="<?php echo ($name); ?>" />
			</div>
			<div class="form-group">
				<label for="">排序</label>
				<input class="form-input-txt" type="text" placeholder="请输入数字排序" name="sort" value="<?php echo ($sort); ?>" />
			</div>
			<div class="form-group">
				<label for="">父分类</label>
				<select name="pid" class="form-select">
					<option value="0">顶级分类</option>
					<?php if(is_array($arr)): $i = 0; $__LIST__ = $arr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($v["id"]); ?>" <?php echo ($pid==$v[id]?selected:''); ?>><?php echo ($v["name"]); ?></option>
						<?php if($v[_child]): if(is_array($v["_child"])): $i = 0; $__LIST__ = $v["_child"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$z): $mod = ($i % 2 );++$i;?><option value="<?php echo ($z["id"]); ?>" <?php echo ($pid==$z[id]?selected:''); ?>>---<?php echo ($z["name"]); ?></option>
								<?php if($z[_child]): if(is_array($z["_child"])): $i = 0; $__LIST__ = $z["_child"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$y): $mod = ($i % 2 );++$i;?><option value="<?php echo ($y["id"]); ?>" <?php echo ($pid==$y[id]?selected:''); ?>>------<?php echo ($y["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
			...
			<div class="form-group" style="margin-left:150px;">
				...
				<button class="return-btn" ><a class="btn-a" href="<?php echo U('Category/index',['pid'=>$pid]);?>">返  回</a></button>
			</div>
		</div>
	</div>
	...
	<script>
		var SCOPE = {
			'redirect_url' : "<?php echo U('Category/editData');?>",
			'jump_url' : "<?php echo U('Category/index');?>?pid="+"<?php echo ($pid); ?>"
		};
		$(function() {
