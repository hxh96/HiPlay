...
<div class="public-ifame mt20">
    ...
    <div class="public-content">
        <div class="public-content-header">
            ...
            <script>
                $(function(){
                    ...
                    $('#searchBtn').click(function(){
                        var phone = $('#phone').val();
                        location.href = "<?php echo U('User/index');?>?phone="+phone;
                    });
                });
            ...
            <div class="public-content-right fr">
                <a href="<?php echo U('User/downUserInfo');?>"  style="height: 24px; width: 60px;border: 1px solid #ccc;font-size: 12px;text-align:center;color: #00C1B3">导出信息</a>
            </div>
            <h3><?php echo ($menu); ?>     真实用户：<?php echo ($userNumber); ?>人</h3>

            <!--<div class="public-content-right fr">-->
                <!--<a href="<?php echo U('User/add');?>" style="height: 24px; width: 60px;border: 1px solid #ccc;font-size: 12px;text-align:center">添加信息</a>-->
            <!--</div>-->
        </div>
        <div class="public-content-cont">
            <table class="public-cont-table">
                ...
                <?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td>
                            <select class="checkUserOfficial" attr-uId="<?php echo ($v["id"]); ?>">
                                <option value="1" <?php echo ($v[is_official]==1?'selected':''); ?>>是</option>
                                <option value="0" <?php echo ($v[is_official]==0?'selected':''); ?>>否</option>
                            </select>
                        </td>
                        <td>
                            <?php if($v[subscribe] == 1): ?><span style="color: #45B549">已关注</span>
                                <?php else: ?>
                                <span style="color: red">未关注</span><?php endif; ?>
                        </td>
                        <td>
                            <?php if($v[is_robot] == 1): ?><span style="color: red">是</span>
                                <?php else: ?>
                                <span style="color: #45B549">否</span><?php endif; ?>
                        </td>
                        <!--<td><?php echo ($v["unionid"]); ?></td>-->
                        <td><a href="<?php echo U('UserPhotoAlbum/index',['user_id'=>$v[id]]);?>" class="number_a"><?php echo ($v["user_photo_album_number"]); ?></a></td>
                        ...
                        <td><img src="<?php echo ($v["head_img"]); ?>" width="50" height="50"/></td>
                        ...
                        <td><a href="<?php echo U('Activity/index',['initiate_user_id'=>$v[id]]);?>" class="number_a"><?php echo ($v["initiate_number"]); ?></a></td>
                        <td><a href="<?php echo U('UserInvited/index',['user_id'=>$v[id]]);?>" class="number_a"><?php echo ($v["invited_number"]); ?></a></td>
                        <td><a href="<?php echo U('UserCollection/index',['user_id'=>$v[id]]);?>" class="number_a"><?php echo ($v["collection_number"]); ?></a></td>
                        <td><a href="<?php echo U('UserCancel/index',['user_id'=>$v[id]]);?>" class="number_a"><?php echo ($v["cancel_number"]); ?></a></td>
                        <td><a href="<?php echo U('UserIntegral/index',['user_id'=>$v[id]]);?>" class="number_a"><?php echo ($v["user_integral_number"]); ?></a></td>
                        <td><?php echo ($v["inputtime"]); ?></td>
                        <td><?php echo ($v["lost_login_time"]); ?></td>
                        ...
                        <td>
                            <div class="table-fun">
                                ...
                                <?php if($v[status] == 1): ?><a href="javaScript:;" attr-id="<?php echo ($v["id"]); ?>" attr-name="封停" class="tp-delete">
    ...
    <script>
        var SCOPE = {
            'redirect_url' : "<?php echo U('User/delete');?>",
            'jump_url' : "<?php echo U('User/index');?>"
        };
            ...
            $('.checkUserOfficial').change(function(){
                ...
                layer.open({
                    ...
                    yes: function () {
                        $.ajax({
                            type:'post',
                            url:"<?php echo U('User/UserOfficial');?>",
