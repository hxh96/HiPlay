...
<div class="public-ifame mt20">
        ...
        <div class="public-content-cont">
            <table class="public-cont-table">
                ...
                <?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
                        <td><?php echo ($v["id"]); ?></td>
                        <td><a href="<?php echo U('User/index',['id'=>$v[user_id]]);?>" style="color: #00C1B3"><?php echo ($v["user_name"]); ?></a></td>
                        <td><?php echo ($v["user_phone"]); ?></td>
                        <td>
                            <?php if($v[status] == 0): ?><span style="color: blue">发起者未响应</span>
                                <?php elseif($v[status] == 1): ?>
                                <span style="color: #6eb6de">发起者接受</span>
                                <?php elseif($v[status] == 2): ?>
                                <span style="color: red">发起者拒绝</span>
                                <?php elseif($v[status] == 3): ?>
                                <span style="color: red">应邀者取消</span>
                                <?php elseif($v[status] == 4): ?>
                                <span style="color: red">发起者取消</span>
                                <?php else: ?>
                                <span style="color: red">应邀超时</span><?php endif; ?>
                        </td>
                        <td><a href="<?php echo U('User/index',['id'=>$v[initiate_user_id]]);?>" style="color: #00C1B3"><?php echo ($v["initiate_user_name"]); ?></a></td>
                        <td><?php echo ($v["initiate_user_phone"]); ?></td>
                        <td title="<?php echo ($v["feedback_content"]); ?>" ><a href="<?php echo U('Activity/index',['id'=>$v[activity_id]]);?>" style="color: #6eb6de"><?php echo ($v["activity_title"]); ?></a></td>
                        <td><?php echo ($v["inputtime"]); ?></td>
                        <!--<td>-->
                            <!--<div class="table-fun">-->
                                <!--&lt;!&ndash;<a href="<?php echo U('Admin/edit',['id'=>$v[id]]);?>">修改</a>&ndash;&gt;-->
                                <!--<?php if($v[status] == 1): ?>-->
                                    <!--<a href="javaScript:;" attr-id="<?php echo ($v["id"]); ?>" attr-name="封停" class="tp-delete">-->
    ...
    <script>
        var SCOPE = {
            'redirect_url' : "<?php echo U('UserFeedback/delete');?>",
            'jump_url' : "<?php echo U('UserFeedback/index');?>"
        };
    </script>
